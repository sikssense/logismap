...
def main():
    ...
    with st.form(key='search_form'):
        ...
        with cols[0]:
            # 시도 선택 (컬럼명 '시도' 사용) - 전체 옵션 제거
            if '시도' in df.columns and df['시도'].notna().sum() > 0:
                regions = sorted(df['시도'].dropna().unique().tolist())  # '전체' 옵션 제거
                selected_region = st.selectbox("시도를 선택하세요.", regions, key="sido")
            else:
                st.text("시도를 선택하세요")
                selected_region = regions[0] if len(regions) > 0 else ''  # 기본값으로 첫 번째 시도 선택
        ...
        with cols[5]:
            ...
            search_submit = st.form_submit_button("검색", on_click=on_search_clicked, type="primary")
